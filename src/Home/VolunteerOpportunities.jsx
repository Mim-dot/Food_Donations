  ...
  return (
    <section className="py-12 bg-[#F5EFE6] dark:bg-gray-900">
      <div className="max-w-6xl mx-auto px-6">
        <motion.h2
          className="text-4xl font-bold mb-10 text-center text-[#7B4F28] dark:text-white"
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {fakeOpportunities.map((opportunity, index) => (
            <motion.div
              key={opportunity.id}
              className="p-6 rounded-2xl border border-[#E0D6CC] bg-white shadow-lg"
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.1, duration: 0.5 }}
            >
              <h3 className="text-2xl font-semibold text-[#7B4F28] mb-3">
                {opportunity.role}
              </h3>
              <p className="flex items-center text-[#5C3B1D] mb-1">
                <FaMapMarkerAlt className="mr-2 text-green-500" /> {opportunity.location}
              </p>
              <p className="flex items-center text-[#5C3B1D] mb-1">
                <FaClock className="mr-2 text-blue-500" /> {opportunity.date} | {opportunity.time}
              </p>
