...
const FeatureDonations = () => {
  ...
  const featureMutation = useMutation({
    mutationFn: ({ donationId, isFeatured }) =>
      axiosSecure.patch(`/admin/donations/${donationId}/feature`, { isFeatured }),
    onSuccess: () => {
    ...
    onError: (error) => {
      ...
      Swal.fire('Error', error.response?.data?.message || 'Failed to update feature status', 'error');
    },
  });
...
<div className="h-screen flex justify-center items-center">
  ...
  if (!donations.length)
    return <div className="text-center py-8"><Useable/></div>;
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {donations.map((donation) => (
              <tr key={donation._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <button
                    onClick={() => handleFeatureToggle(donation._id, donation.isFeatured)}
